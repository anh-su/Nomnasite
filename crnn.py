try:
    ...
    # Chạy ngoài Streamlit (training script) — dùng no-op decorator
    def _CACHE(**_kw):
        return lambda fn: fn
    st = None
...
class CRNN(tf.keras.Model):
    ...
    def predict_one_patch(_self, patch_img):
        ...
        # confidence: trung bình xác suất cao nhất tại mỗi timestep
        max_probs = pred_tokens[0].max(axis=-1)
        confidence = round(float(max_probs.mean()) * 100, 1)
        return pred_labels[0], confidence